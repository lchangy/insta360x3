...
private:
    ...
    void InitFFmpegDecoder() {
        if (!use_hardware_decoder_) {
            ...
            if (!codec_) {
                RCLCPP_ERROR(this->get_logger(), "No software H.264 decoder available");
                return;
            }
        } else {
    ...
    void compressed_image_callback(const sensor_msgs::msg::CompressedImage::SharedPtr msg) {
        ...
        if (!codec_ctx_ || !pkt_ || !hw_frame_ || msg->data.empty()) {
            return;
        }

        // CameraSDK delivers one complete Annex-B access unit per callback.
        // Feeding those units through av_parser_parse2 split the SPS/PPS from
        // the IDR frame on the X3 1440x720 stream, so the decoder never learned
        // PPS 0.  Preserve the callback boundary and submit one owned packet.
        av_packet_unref(pkt_);
        if (av_new_packet(pkt_, static_cast<int>(msg->data.size())) < 0) {
            return;
        }
        std::memcpy(pkt_->data, msg->data.data(), msg->data.size());
...
public:
    H264DecoderNode() : Node("h264_decoder_node") {
        ...
        this->declare_parameter("use_hardware_decoder", false);
        ...
        use_hardware_decoder_ = this->get_parameter("use_hardware_decoder").as_bool();
