...
- surround (default="false")

When enabled together with `equirectangular`, this publishes four perspective surround views:
`/surround/front/image`, `/surround/right/image`, `/surround/back/image`, and `/surround/left/image`.
For example:

```
ros2 launch insta360_ros_driver bringup.launch.xml equirectangular:=true surround:=true
```
...
The decoder uses software H.264 decoding by default for compatibility. If a compatible NVDEC/CUDA stack is available, enable it with `hardware_decoder:=true`.
